# 🔵 คู่มือสำหรับ SHOGUN — Lexer Developer
...
งานของฉันคือเขียนไฟล์ lexer.c ซึ่งเป็นตัว Tokenizer

หน้าที่ของ Lexer:
- รับ source code (string) แล้ว scan ทีละตัวอักษร
- สร้าง Token Linked List (head → tail)
- แยก keywords, identifiers, literals, operators

นี่คือไฟล์ lexer.h (Interface ที่ต้อง implement ตาม — ห้ามแก้ไข):

[วางโค้ด src/lexer.h ทั้งไฟล์ที่นี่]

นี่คือไฟล์ lexer.c ที่มี skeleton/TODO อยู่แล้ว:

[วางโค้ด src/lexer.c ทั้งไฟล์ที่นี่]

ช่วย implement ทุก function ใน lexer.c ให้ครบตาม TODO ที่เขียนไว้
โดยต้อง:
1. ใช้ function signature ตรงตาม lexer.h (ห้ามเปลี่ยน)
2. Implement token_list_create, token_list_append, token_list_free,
   token_type_name, และ lexer_tokenize
3. lexer_tokenize ต้อง:
   - ข้าม whitespace (space, tab, \r)
   - จัดการ newline (\n) → TOKEN_NEWLINE (ข้ามซ้ำ)
   - ข้าม comment (// จนจบบรรทัด)
   - scan ตัวเลข → TOKEN_INT_LIT
   - scan string ใน "..." → TOKEN_STRING_LIT
   - scan identifier/keyword → match กับ keyword table:
     set, as, to, show, if, else, while, then, do, end, int, string
   - scan operators: +, -, *, /, ==, >, <, (, )
   - ปิดท้ายด้วย TOKEN_EOF
4. compile ได้ด้วย gcc -Wall -Wextra
5. คืนเฉพาะโค้ดไฟล์ lexer.c เท่านั้น
```
